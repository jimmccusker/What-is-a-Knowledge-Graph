...
While the use of ``limited set of relation types'' was focused on a specific set of non-decomposable relations above, in the context of an open world knowledge system this should be taken to mean a core set of relations and classes that subsume or can be used to compose any other key relations and classes.   
...
In practice, the knowledge graph papers and their related knowledge graphs reviewed seem to all aggregate knowledge from many secondary sources and either use Natural Language Processing (NLP) extraction techniques when the sources are unstructured text, or use a semantic Extraction Transformation, and Load process from structured databases \cite{McCusker_2009}.
